...
public class ScriptReader implements Closeable {
	 ...
	 *
	 * NOTE: This static variable was added by FBSQL Team team
	 */
	...
	public ScriptReader(Reader reader) {
		this.reader = reader;
		buffer      = new char[IO_BUFFER_SIZE * 2]; // NOTE: This line was modified by FBSQL Team team
	}
...
//    }
	 ...
	 * @throws IOException
	 *
	 * NOTE: This method was modified by FBSQL Team team (for original version see above)
	 */
	@Override
...
//  }
	...
	private int readBuffer() throws IOException {
		...
		if (keep > 0) {
			char[] src = buffer;
			if (keep + IO_BUFFER_SIZE > src.length) { // NOTE: This line was modified by FBSQL Team team
				// protect against NegativeArraySizeException
				if (src.length >= Integer.MAX_VALUE / 2) {
					throw new IOException("Error in parsing script, " + "statement size exceeds 1G, " + "first 80 characters of statement looks like: " + new String(buffer, bufferStart, 80));
				}
		...
		bufferStart  = 0;
		bufferPos    = keep;
		int len = reader.read(buffer, keep, IO_BUFFER_SIZE); // NOTE: This line was modified by FBSQL Team team
		if (len == -1) {
